...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('Initial display desktopChatToggleBtn:', desktopChatToggleBtn ? desktopChatToggleBtn.style.display : 'N/A');
    console.log('Initial display mobileChatToggleBtn:', mobileChatToggleBtn ? mobileChatToggleBtn.style.display : 'N/A');
    ...
    function sendChatMessage() {
        ...
        if (message) {
            socket.emit('chat_message', { room_id: roomId, player_id: playerId, message: message });
            chatInput.value = '';
        }
    }
    ...
    function sendDesktopChatMessage() {
        const message = chatInput.value.trim();
        if (message) {
            socket.emit('chat_message', { room_id: roomId, player_id: playerId, message: message });
            chatInput.value = '';
    ...
    mobileChatToggleBtn.addEventListener('click', () => {
        console.log('mobileChatToggleBtn clicked.');
        console.log('mobileChatPanel classList before toggle:', mobileChatPanel.classList.value);
        mobileChatPanel.classList.toggle('open');
        console.log('mobileChatPanel classList after toggle:', mobileChatPanel.classList.value);
        chatNotificationDot.style.display = 'none';
        console.log('chatNotificationDot display set to none.');
    });

    closeChatBtn.addEventListener('click', () => {
        console.log('closeChatBtn clicked.');
        console.log('mobileChatPanel classList before remove open:', mobileChatPanel.classList.value);
        mobileChatPanel.classList.remove('open');
        console.log('mobileChatPanel classList after remove open:', mobileChatPanel.classList.value);
    });

    desktopChatToggleBtn.addEventListener('click', () => {
        console.log('desktopChatToggleBtn clicked.');
        console.log('desktopChatPanel classList before toggle:', desktopChatPanel.classList.value);
        desktopChatPanel.classList.toggle('open');
        console.log('desktopChatPanel classList after toggle:', desktopChatPanel.classList.value);
        desktopChatNotificationDot.style.display = 'none';
        console.log('desktopChatNotificationDot display set to none.');
    });

    closeDesktopChatBtn.addEventListener('click', () => {
        console.log('closeDesktopChatBtn clicked.');
        console.log('desktopChatPanel classList before remove open:', desktopChatPanel.classList.value);
        desktopChatPanel.classList.remove('open');
        console.log('desktopChatPanel classList after remove open:', desktopChatPanel.classList.value);
    });
    ...
    function initializeGame(newRoomId, newPlayerId, puzzle, difficulty) {
        ...
        // Enable chat input/send when entering waiting room
    ...
    socket.on('game_started', (data) => {
        console.log('Game started, start_time:', data.start_time); // Log start_time
        ...
        function updateUI() {
            ...
            const chatInputs = document.querySelectorAll('#chat-input, #chat-send-btn, #mobile-chat-input, #mobile-chat-send-btn');
            ...
            if (playerState === 'playing') {
                ...
                if(mobileChatToggleBtn) mobileChatToggleBtn.style.display = 'none';
        ...
        socket.on('chat_message', (data) => {
            ...
            console.log('mobileChatPanel open:', mobileChatPanel ? mobileChatPanel.classList.contains('open') : 'N/A');
            console.log('desktopChatPanel open:', desktopChatPanel ? desktopChatPanel.classList.contains('open') : 'N/A');

            if (mobileChatPanel && !mobileChatPanel.classList.contains('open')) {
                chatNotificationDot.style.display = 'block';
                console.log('Mobile notification dot set to block.');
            }

            if (desktopChatPanel && !desktopChatPanel.classList.contains('open')) {
                desktopChatNotificationDot.style.display = 'block';
                console.log('Desktop notification dot set to block.');
            }
        });
    }
    ...
    function transitionToWaitingRoom() {
        ...
        // Show mobile chat toggle on mobile, desktop chat toggle on desktop
        if (window.innerWidth < 992) { // Bootstrap's lg breakpoint is 992px
            if(mobileChatToggleBtn) mobileChatToggleBtn.style.display = 'block';
            if(desktopChatToggleBtn) desktopChatToggleBtn.style.display = 'none';
            if (chatPanel && mobileChatPanel) {
                mobileChatPanel.appendChild(chatPanel);
                chatPanel.style.display = 'block';
            }
        } else {
            if(mobileChatToggleBtn) mobileChatToggleBtn.style.display = 'none';
            if(desktopChatToggleBtn) desktopChatToggleBtn.style.display = 'block';
            if (chatPanel && desktopChatPanel) {
                desktopChatPanel.appendChild(chatPanel);
                chatPanel.style.display = 'block';
            }
        }
        console.log('After transitionToWaitingRoom - desktopChatToggleBtn display:', desktopChatToggleBtn ? desktopChatToggleBtn.style.display : 'N/A');
        console.log('After transitionToWaitingRoom - mobileChatToggleBtn display:', mobileChatToggleBtn ? mobileChatToggleBtn.style.display : 'N/A');
        updateStats(0, 0, 0);
    }
    ...
    function transitionToGameView(isSoloParam) {
        ...
        if (isSoloParam) {
            ...
            // Hide all chat elements in solo mode
            if(mobileChatToggleBtn) mobileChatToggleBtn.style.display = 'none';
            if(desktopChatToggleBtn) desktopChatToggleBtn.style.display = 'none';
            if(chatPanel) chatPanel.style.display = 'none';
            chatInput.disabled = true; // Disable chat input in solo mode
            chatSendBtn.disabled = true; // Disable chat send button in solo mode
        } else {
            ...
            // For multiplayer game view, disable chat input/send for fixed chatbox
            chatInput.disabled = true;
            chatSendBtn.disabled = true;
            // Mobile chat toggle button remains visible, desktop chat toggle button is hidden
            if(mobileChatToggleBtn) mobileChatToggleBtn.style.display = 'block';
            if(desktopChatToggleBtn) desktopChatToggleBtn.style.display = 'none'; 
            if(chatPanel) chatPanel.style.display = 'block'; // Ensure fixed chat panel is visible
        ...
        console.log('After transitionToGameView - desktopChatToggleBtn display:', desktopChatToggleBtn ? desktopChatToggleBtn.style.display : 'N/A');
        console.log('After transitionToGameView - mobileChatToggleBtn display:', mobileChatToggleBtn ? mobileChatToggleBtn.style.display : 'N/A');
        console.log('After transitionToGameView - chatInput disabled:', chatInput.disabled);
        console.log('After transitionToGameView - chatSendBtn disabled:', chatSendBtn.disabled);
        updateStats(0, 0, 0);
    }
    ...
    function transitionToRoomView() {
        ...
        console.log('After transitionToRoomView - desktopChatToggleBtn display:', desktopChatToggleBtn ? desktopChatToggleBtn.style.display : 'N/A');
        console.log('After transitionToRoomView - mobileChatToggleBtn display:', mobileChatToggleBtn ? mobileChatToggleBtn.style.display : 'N/A');
        stopTimer();
        resetGameState();
        playSoloBtn.disabled = false;
    }
