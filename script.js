...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const preChatMessages = document.getElementById('pre-chat-messages');
    const mobilePreChatMessages = document.getElementById('mobile-pre-chat-messages');

    const preDefinedMessages = ["Good luck!", "Nice move!", "I'm stuck.", "Well played!", "gg"];
    ...
    function renderPreChatMessages() {
        preChatMessages.innerHTML = '';
        mobilePreChatMessages.innerHTML = '';
        preDefinedMessages.forEach(msg => {
            const btn = document.createElement('button');
            btn.textContent = msg;
            btn.classList.add('btn', 'btn-outline-secondary', 'btn-sm', 'me-1', 'mb-1');
            btn.addEventListener('click', () => {
                socket.emit('chat_message', { room_id: roomId, player_id: playerId, message: msg });
            });
            preChatMessages.appendChild(btn);

            const mobileBtn = btn.cloneNode(true);
            mobileBtn.addEventListener('click', () => {
                socket.emit('chat_message', { room_id: roomId, player_id: playerId, message: msg });
            });
            mobilePreChatMessages.appendChild(mobileBtn);
        });
    }
    ...
    function connectWebSocket() {
        ...
        socket.on('game_over', (data) => {
            stopTimer();
            document.querySelectorAll('.number-button, #actions-container button, #mobile-controls button, #chat-input, #chat-send-btn, #mobile-chat-input, #mobile-chat-send-btn').forEach(button => {
                button.disabled = true;
            });
            if (isSolo) {
    ...
    function handleNumberInput(e) {
        if (!isGameActive || isPaused || !selectedCell || selectedCell.classList.contains('fixed')) return;
    ...
    function isChatFocused() {
        return document.activeElement === chatInput || document.activeElement === mobileChatInput;
    }
    ...
    function transitionToGameView(isSoloParam) {
        ...
        if (isSoloParam) {
            ...
            if(mobileInfoTabs) mobileInfoTabs.style.display = 'block'; // Keep tabs container visible
            if(mainHeader) mainHeader.style.display = 'none';

            // Hide players tab and show stats tab
            if(playersTab) playersTab.style.display = 'none';
            ...
            // Ensure both tabs are visible for multiplayer
            if(playersTab) playersTab.style.display = 'block';
            // Default to stats tab active for multiplayer on game start
            if(statsTab) statsTab.classList.add('active');
