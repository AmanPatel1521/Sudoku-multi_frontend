...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function connectWebSocket() {
        ...
        socket.on('game_state_update', (data) => {
            ...
            if (data.last_move) {
                ...
                // Highlight all instances of the inserted number
                if (value !== 0) {
                    highlightActiveNumbers(value);
                } else {
                    clearHighlights(); // Clear highlights if the value is erased
                }
            }
        });
    ...
    const selectCell = (cell) => {
        ...
        // Highlight numbers if the selected cell has a value and is not in notes mode
        ...
        // Synchronize notes from currentNotesBoard to selectedCell.dataset.notes
        const r = parseInt(selectedCell.dataset.row);
        const c = parseInt(selectedCell.dataset.col);
    ...
    const renderNotes = (cell, notes) => {
        ...
        if (notes.length > 0) {
            ...
            cell.classList.add('has-notes'); // Ensure this class is added
        } else {
    ...
    function updatePlayerList() {
        const oldPlayerOrder = playersInRoom.map(p => p.player_id);

        playerListUl.innerHTML = '';
        playersInRoom.forEach((player, newIndex) => {
            const listItem = document.createElement('li');
            listItem.className = 'list-group-item d-flex justify-content-between align-items-center';
            listItem.dataset.playerId = player.player_id;

            const playerNameSpan = document.createElement('span');
            playerNameSpan.textContent = player.player_name + (player.player_id === playerId ? ' (You)' : '');
            
            const playerScoreSpan = document.createElement('span');
            playerScoreSpan.className = 'badge bg-primary rounded-pill';
            ...
            const oldIndex = oldPlayerOrder.indexOf(player.player_id);
            if (oldIndex !== -1 && oldIndex !== newIndex) {
                const animationClass = newIndex < oldIndex ? 'player-move-up' : 'player-move-down';
                listItem.classList.add(animationClass);
                listItem.addEventListener('animationend', () => {
                    listItem.classList.remove(animationClass);
                });
            }
        });
    }
