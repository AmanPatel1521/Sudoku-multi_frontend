...
const BACKEND_URL = 'https://sudoku-multi-backend.onrender.com';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const gameStatsMobile = document.getElementById('game-stats-mobile');
    ...
    async function handleJoinRoom() {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${BACKEND_URL}/join_room`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            initializeGame(data.room_id, data.player_id, data.puzzle, 'N/A');
        } catch (error) {
    ...
    function connectWebSocket() {
        ...
        socket.on('player_joined', (data) => {
            playersInRoom[data.player_id] = { name: data.player_name, eliminated: false };
            updatePlayerList();
            showTemporaryMessage(`${data.player_name} joined the room.`);
        });
        ...
        socket.on('player_eliminated', (data) => {
            if (playersInRoom[data.player_id]) {
                playersInRoom[data.player_id].eliminated = true;
            }
            updatePlayerList();
    ...
    function updateStats(mistakes, hints) {
        if (mistakes !== null && mistakes !== undefined) {
            const mistakeText = `${mistakes} / 3`;
            mistakesCounter.textContent = mistakeText;
            if (gameStatsMobile) gameStatsMobile.querySelector('#mistakes-counter').textContent = mistakeText;
        }
        if (hints !== null && hints !== undefined) {
            const hintsLeft = 3 - hints;
            hintButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-lightbulb-fill" viewBox="0 0 16 16"><path d="M2 6a6 6 0 1 1 10.174 4.31c-.203.196-.359.4-.453.619l-.762 1.769A.5.5 0 0 1 10.5 13h-5a.5.5 0 0 1-.46-.302l-.761-1.77a1.99 1.99 0 0 0-.453-.618A6 6 0 0 1 2 6m3 8.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1l-.224.447a1 1 0 0 1-.894.553H6.618a1 1 0 0 1-.894-.553L5.5 15a.5.5 0 0 1-.5-.5"/></svg> Hint (${hintsLeft} left)`;
    ...
    function startTimer() {
        ...
        timerInterval = setInterval(() => {
            if (!isPaused) {
                ...
                if (gameStatsMobile) gameStatsMobile.querySelector('#timer').textContent = timeString;
            }
        }, 1000);
    }
    ...
    function togglePause() {
        isPaused = !isPaused;
        pauseButton.innerHTML = isPaused ? `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-play-fill" viewBox="0 0 16 16"><path d="m11.596 8.697-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393"/></svg> Resume` : `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pause-fill" viewBox="0 0 16 16"><path d="M5.5 5.5A.5.5 0 0 1 6 6v4a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m4 0a.5.5 0 0 1 .5.5v4a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5"/></svg> Pause`;
    ...
    function transitionToGameView() {
        ...
        startTimer();
        if (gameStatsMobile) {
            gameStatsMobile.innerHTML = `
                <div id="timer-container" class="flex-grow-1">
                    <span class="label">Time</span>
                    <div id="timer" class="fs-4">00:00</div>
                </div>
                <div id="mistakes-container" class="flex-grow-1">
    ...
    function resetGameState() {
        ...
        if (gameStatsMobile) gameStatsMobile.querySelector('#timer').textContent = '00:00';
        board.innerHTML = '';
    }
