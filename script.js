...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleCreateRoom() {
        ...
        try {
            ...
            isHost = true;
            initializeGame(data.room_id, data.player_id, data.puzzle, data.difficulty, false);
        } catch (error) {
    ...
    function initializeGame(newRoomId, newPlayerId, puzzle, difficulty, isSolo) {
        roomId = newRoomId;
        ...
        connectWebSocket(isSolo);
    }

    function connectWebSocket(isSolo = false) {
        ...
        socket.on('current_players', (data) => {
            ...
            if (isHost && !isSolo) {
                startGameBtn.disabled = playersInRoom.length < 1;
            }
        });
