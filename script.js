...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateGameInfo(roomId, difficulty, isSoloParam) {
        ...
        const mobileStatLabels = document.querySelectorAll('#game-stats-mobile .label');

        if (isSoloParam) {
            roomIdDisplay.parentElement.style.display = 'none';
            mobileStatLabels.forEach(label => label.style.display = 'none');
        }
        else {
            roomIdDisplay.parentElement.style.display = 'flex';
            roomIdDisplay.textContent = roomId;
            mobileStatLabels.forEach(label => label.style.display = 'block');
        }
