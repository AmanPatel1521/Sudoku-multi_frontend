...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleCreateRoom() {
        ...
        try {
            ...
            isHost = true;
            initializeGame(data.room_id, data.player_id, data.puzzle, data.difficulty, data.time_limit, false);
        } catch (error) {
    ...
    async function handlePlaySolo() {
        ...
        try {
            ...
            isHost = true;
            initializeGame(data.room_id, data.player_id, data.puzzle, difficulty, null, true);
        } catch (error) {
            console.error('Error creating room:', error);
            alert(`Failed to create room: ${error.message}`);
        } finally {
    ...
    function initializeGame(newRoomId, newPlayerId, puzzle, difficulty, timeLimit, gameStarted) {
        roomId = newRoomId;
        ...
        updateGameInfo(roomId, difficulty, gameStarted);
        connectWebSocket();
    }
    ...
    function connectWebSocket() {
        ...
        socket.on('game_started', (data) => {
            ...
            startCountdownTimer(data.time_limit);
        });
        ...
        socket.on('current_players', (data) => {
            ...
            if (isHost) {
                startGameBtn.disabled = playersInRoom.length < 1; // Changed to 1 for easier testing
            }
        });
    ...
    function updatePlayerList() {
        ...
        playersInRoom.forEach((player, index) => {
            ...
            listItem.querySelector('.player-name').textContent = player.player_name + (player.player_id === playerId ? ' (You)' : '');
    ...
    function transitionToGameView(isSolo = false) {
        ...
        if (isSolo) {
            ...
            startCountdownTimer(3600); // 1 hour for solo
        } else {
    ...
    function updateGameInfo(roomId, difficulty, isSolo) {
        if (isSolo) {
