...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleNumberInput(e) {
        if (isPaused || !selectedCell || selectedCell.classList.contains('fixed') || eraserMode) return;
    ...
    const selectCell = (cell) => {
        ...
        clearHighlights();
        selectedCell = null; // Deselect first
    ...
    function updateStats(mistakes, hints, score) {
        ...
        if (hints !== null && hints !== undefined) {
            ...
            if (hintsLeft > 0) {
                hintCountSpan.style.display = 'block';
            } else {
                hintCountSpan.style.display = 'none';
            }
    ...
    function showEliminationOverlay(message) {
        ...
        const randomEliminationMessage = eliminationMessages[Math.floor(Math.random() * eliminationMessages.length)];
